...
from fla.ops.bla_kv.chunk_bla_h import (chunk_bla_bwd_dhu,
                                          chunk_bla_fwd_h)
from fla.ops.bla_kv.chunk_o import (chunk_bwd_dqkwg, chunk_bwd_dv_local,
                                    chunk_fwd_o)
from fla.ops.bla_kv.wy_fast import (bwd_prepare_wy_repr,
                                              fwd_prepare_wy_repr,
                                              fwd_recompute_w_u)
...
def chunk_bla_fwd(
    ...
    # safe_exp
    h, v_new, final_state = chunk_bla_fwd_h(
        k=k,
        w=w,
        u=u,
        g=g,
...
class ChunkBLAFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        g, o, Aw, Au, final_state = chunk_bla_fwd(
            q=q,
            k=k,
            v=v,
            g=g,
    ...
    def backward(
        ...
        if ctx.use_qk_l2norm_in_kernel:
            ...
            v, v_orig = l2norm_fwd(v), v
        dq, dk, dv, db, dg, dh0 = chunk_bla_bwd(
            q=q,
            k=k,
            v=v,
            g=g,
...
def chunk_bla_kv(
    ...
    assert k.shape[-1] == v.shape[-1], "The dimension of keys and values must be the same."

    if initial_state is not None:
        initial_state = initial_state.to(torch.float32)

    o, final_state = ChunkBLAFunction.apply(
        q,
        k,
        v,
        g,
