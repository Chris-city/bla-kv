...
def fwd_prepare_wy_repr_kernel_chunk64(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        if HEAD_FIRST:
            ...
            p_v = tl.make_block_ptr(v + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BC, BK), (1, 0))
            p_k2 = tl.make_block_ptr(k + i_bh * T * K, (T, K), (K, 1), (i_t * BT + BC, i_k * BK), (BC, BK), (1, 0))
            p_v2 = tl.make_block_ptr(v + i_bh * T * K, (T, K), (K, 1), (i_t * BT + BC, i_k * BK), (BC, BK), (1, 0))
        else:
            p_k = tl.make_block_ptr(k + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BC, BK), (1, 0))
            p_v = tl.make_block_ptr(v + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BC, BK), (1, 0))
            p_k2 = tl.make_block_ptr(k + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT + BC, i_k * BK), (BC, BK), (1, 0))
            p_v2 = tl.make_block_ptr(v + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT + BC, i_k * BK), (BC, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        b_v = tl.load(p_v, boundary_check=(0, 1))
        b_vb = (b_v * b_beta[:, None]).to(b_v.dtype)
        b_k2 = tl.load(p_k2, boundary_check=(0, 1))
        b_v2 = tl.load(p_v2, boundary_check=(0, 1))
        b_vb2 = (b_v2 * b_beta2[:, None]).to(b_v2.dtype)
        b_Aw += tl.dot(b_vb, tl.trans(b_k))
        b_Aw2 += tl.dot(b_vb2, tl.trans(b_k2))
        b_Aw3 += tl.dot(b_vb2, tl.trans(b_k))
...
def fwd_prepare_wy_repr(
    ...
    assert BT == 64, 'Only BT=64 is supported for now.'
...
def bwd_prepare_wy_repr_kernel(
    ...
    for i_k in range(tl.cdiv(K, BK)): # for W
        if HEAD_FIRST:
            p_k = tl.make_block_ptr(k + i_bh * T*K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    ...
    for i_k in range(tl.cdiv(K, BK)):
        if HEAD_FIRST:
            p_k = tl.make_block_ptr(k + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_v = tl.make_block_ptr(v + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_dk = tl.make_block_ptr(dk + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_dv = tl.make_block_ptr(dv + i_bh * T * K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        else:
            p_k = tl.make_block_ptr(k + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_v = tl.make_block_ptr(v + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_dk = tl.make_block_ptr(dk + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_dv = tl.make_block_ptr(dv + (bos * H + i_h) * K, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        b_k = tl.load(p_k, boundary_check=(0, 1))
        b_v = tl.load(p_v, boundary_check=(0, 1))
        b_dk = tl.load(p_dk, boundary_check=(0, 1))
        b_dv = tl.load(p_dv, boundary_check=(0, 1))
        b_v_beta = (b_v * b_beta[:, None]).to(b_k.dtype)
        b_A += tl.dot(b_v_beta, tl.trans(b_k))
        b_dv_beta = tl.dot(b_dA, b_k, allow_tf32=False)
        b_dbeta += tl.sum(b_dv_beta * b_v, 1)
        b_dk += tl.dot(tl.trans(b_dA), b_v_beta, allow_tf32=False)
        b_dv += b_dv_beta * b_beta[:, None]
        tl.store(p_dk, b_dk.to(p_dk.dtype.element_ty), boundary_check=(0, 1))
        tl.store(p_dv, b_dv.to(p_dv.dtype.element_ty), boundary_check=(0, 1))
